...
export function BorrowedListTab() {
  ...
  useEffect(() => {
    const fetchLoans = async () => {
      ...
      try {
        ...
        const response = await fetch('https://library-backend-production-b9cf.up.railway.app/api/me/loans?page=1&limit=50', {
          headers: {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://library-backend-production-b9cf.up.railway.app/api/reviews', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    } catch {  // <-- Kosongkan saja kurung kurawalnya
  ...
  const calculateDuration = (borrowedAt: string, dueAt: string) => {
    const diffTime = Math.abs(new Date(dueAt).getTime() - new Date(borrowedAt).getTime());
  ...
  return (
    <div className="flex flex-col items-start gap-[16px] md:gap-[24px] w-full animate-in fade-in duration-300">
      ...
      <h2 className="font-bold text-[24px] md:text-[28px] text-[#0A0D12]">Borrowed List</h2>
      ...
      <div className="flex flex-col gap-[16px] w-full mt-2">
        {loading ? (
          ...
          filteredLoans.map((loan) => (
            <div key={loan.id} className="flex flex-col p-[16px] md:p-[20px] gap-[16px] md:gap-[20px] w-full bg-white shadow-[0px_0px_20px_rgba(203,202,202,0.25)] rounded-[16px]">
              
              {/* Header Status & Due Date */}
              <div className="flex flex-row items-center justify-between w-full">
                <div className="flex items-center gap-[8px] md:gap-[12px]">
                  <span className="font-bold text-[14px] md:text-[16px] text-[#0A0D12]">Status</span>
                  {loan.status === 'BORROWED' && (
                    <div className="px-[8px] py-[2px] bg-[rgba(36,165,0,0.05)] rounded-[4px]">
                      <span className="font-bold text-[14px] text-[#24A500]">Active</span>
                    </div>
                  )}
                  {loan.status === 'LATE' && (
                    <div className="px-[8px] py-[2px] bg-[rgba(238,29,82,0.1)] rounded-[4px]">
                      <span className="font-bold text-[14px] text-[#EE1D52]">Late</span>
                    </div>
                  )}
                  {loan.status === 'RETURNED' && (
                    <div className="px-[8px] py-[2px] bg-gray-100 rounded-[4px]">
                      <span className="font-bold text-[14px] text-gray-600">Returned</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-[8px] md:gap-[12px]">
                  ...
                  <div className="px-[8px] py-[2px] bg-[rgba(238,29,82,0.1)] rounded-[4px]">
                    <span className="font-bold text-[14px] text-[#0A0D12]">
                      {loan.status === 'RETURNED' ? formatDate(loan.returnedAt!) : formatDate(loan.dueAt)}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center w-full gap-[16px]">
                ...
                <div className="flex flex-row items-center gap-[16px] flex-1">
                  <img 
                    src={loan.book?.coverImage || "https://images.unsplash.com/photo-1544947950-fa07a98d237f?q=80&w=200"} 
                    alt={loan.book?.title} 
                  ...
                  <div className="flex flex-col items-start gap-[4px]">
                    <div className="border border-[#D5D7DA] rounded-[6px] px-[8px] h-[28px] flex items-center justify-center">
                      <span className="font-bold text-[14px] text-[#0A0D12] line-clamp-1">{loan.book?.category?.name || "General"}</span>
                    </div>
                    <h3 className="font-bold text-[16px] md:text-[20px] text-[#0A0D12] line-clamp-1">{loan.book?.title}</h3>
                    <p className="font-medium text-[14px] md:text-[16px] text-[#414651] line-clamp-1">{loan.book?.author?.name || "Unknown Author"}</p>
                    <div className="flex items-center gap-[8px] mt-1">
                      <span className="font-bold text-[14px] md:text-[16px] text-[#0A0D12]">{formatDate(loan.borrowedAt)}</span>
                      <div className="w-[4px] h-[4px] rounded-full bg-[#0A0D12]"></div>
                      <span className="font-bold text-[14px] md:text-[16px] text-[#0A0D12]">
                        Duration {calculateDuration(loan.borrowedAt, loan.dueAt)}
                      </span>
                    </div>
                  </div>
                </div>
      ...
      {isReviewModalOpen && (
        <div className="fixed inset-0 bg-[#0A0D12]/50 z-[100] flex items-center justify-center p-[16px] font-['Quicksand'] animate-in fade-in duration-200">
          <div className="flex flex-col items-center p-[16px] md:p-[24px] gap-[24px] w-full max-w-[345px] md:max-w-[439px] bg-white rounded-[16px] shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="flex flex-row items-center justify-between w-full">
              <h2 className="font-bold text-[18px] md:text-[24px] text-[#0A0D12]">Give Review</h2>
              <button onClick={() => setIsReviewModalOpen(false)} className="text-[#0A0D12] hover:text-[#EE1D52] transition-colors outline-none">
                <X className="w-[24px] h-[24px]" strokeWidth={2} />
              </button>
            </div>

            <div className="flex flex-col items-center w-full">
              <span className="font-bold text-[14px] md:text-[16px] text-[#0A0D12] mb-[4px]">Give Rating</span>
                ...
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star 
                    ...
                    className={`w-[40px] h-[40px] md:w-[49px] md:h-[49px] cursor-pointer transition-colors ${
                      star <= rating ? 'fill-[#FDB022] text-[#FDB022]' : 'fill-[#A4A7AE] text-[#A4A7AE]'
                    }`} 
                  />
                ))}
              </div>
